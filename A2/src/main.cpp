...
std::string optimizationHeaderFile() {
    
    return std::string(DataDir(A2_INCLUDE_DIR)) + "/"+std::string(DataDir(OPTIM_HEADER_FILE));
}

std::string optimizationInlFile() {
    
    return std::string(DataDir(A2_SRC_DIR)) + "/"+std::string("Optimization.inl");
}

int main(int argc, char **argv) {
    
    std::cout<<optimizationHeaderFile()<<"\n";
    ...
    menu.callback_draw_custom_window = [&]()
    {
        ...
        if (ImGui::Button("Load Script", ImVec2(-1,0)))
        {
            ...
            system((std::string("../interpreter/bin/Interpreter ") + currentFilename+" "+optimizationHeaderFile()+" "+optimizationInlFile()).c_str());
